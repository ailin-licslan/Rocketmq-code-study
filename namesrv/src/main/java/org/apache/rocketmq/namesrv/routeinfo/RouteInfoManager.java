...
public class RouteInfoManager {
    ...
    /***
     * 至于RouteInfoManager的注册Broker的方法，我们
     * 可以自己到RouteInfoManager的注册Broker的方法里去看看，
     * 最终如何把一个Broker机器的数据放入
     * RouteInfoManager中维护的路由数据表里去的。
     * 核心思路是用一些Map类的数据结构，
     * 去存放你的Broker的路由数据就可以了，包括了Broker的clusterName、
     * brokerId、brokerName这些核心数据。
     * 而且在更新的时候，一定会基于Java并发包下的ReadWriteLock进行读写锁加锁，
     * 因为在这里更新那么多的内存Map数据结构，必须要加一个写锁，此时只能有一个线程来更新他们才行！
     * */
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                //每隔30s封装一个新的BrokerLiveInfo 带有时间戳的  覆盖更新之前的
    ...
    public void scanNotActiveBroker() {
        ...
        while (it.hasNext()) {
            ...
            //时间差对比 距离上一次心跳时间差值是多少  超过默认120s 就认为挂了
            //private final static long BROKER_CHANNEL_EXPIRED_TIME = 1000 * 60 * 2;
