...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        ...
        //发送心跳给所有的broker  这个netty client行为  此时 broker就作为netty server了
    ...
    private SendResult sendDefaultImpl(
        ...
        //检查一下要发消息的那个topic的路由数据是否在本地  如果去name server 拉取并缓存一下
        TopicPublishInfo topicPublishInfo = this.tryToFindTopicPublishInfo(msg.getTopic());

        /**
         * 当你拿到了一个Topic的路由数据之后，
         * 其实接下来就应该选择要发送消息到这个Topic的哪一个MessageQueue上去了！
         * 因为大家都知道，Topic是一个逻辑上的概念，一个Topic的数据往往是分布式存储在
         * 多台Broker机器上的，因此Topic本质是由多个MessageQueue组成的。
         * 每个MessageQueue都可以在不同的Broker机器上，当然也可能一个Topic的多个
         * MessageQueue在一个Broker机器上
         * */
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                /**
                 * 在这个方法里，只要你获取到了Topic的路由数据，不管从本地缓存获取的，
                 * 还是从NameServer拉取到的，接着就会执行下面的核心代码。
                 * */
    ...
    //去name server 拉取 topic信息  发送网络请求 netty client  如果本地客户端没有找到
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        //根据brokerName先去本地找实际的地址
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(mq.getBrokerName());
        if (null == brokerAddr) {
            //找不到再去name server 拉去一把
