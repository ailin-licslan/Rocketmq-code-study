...
public class BrokerController {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        ...
        if (result) {
            try {
                ...
                //如果启动了Dleger 主从同步技术 以及commitLog的方式 (顺序写+pageCahe)
        ...
        //result == true
        if (result) {
            //netty 服务器 和name Server 基本一样的套路
    ...
    /**
     * 你知道Broker里有这么一些核心组件，都进行了初始化以及完成了启动，但
     * 是我们应该最主要关注的事情是这么几个：
     * （1）Broker启动了，必然要去注册自己到NameServer去，所以BrokerOuterAPI
     * 这个组件必须要画到自己的图里去，这是一个核心组件
     * （2）Broker启动之后，必然要有一个网络服务器去接收别人的请求(生产者|消费组)，此时NettyServer这个组件是必须要知道的
     * （3）当你的NettyServer接收到网络请求之后，需要有线程池来处理，你需要知道这里应该有一个处理各种请求的线程池
     * （4）你处理请求的线程池在处理每个请求的时候，是不是需要各种核心功能组件的协调？比如写入消息到commitlog，然后
     * 写入索引到indexfile和consumer queue文件里去，此时你是不是需要对应的一些MessageStore之类的组件来配合你？
    ...
    public void start() throws Exception {
        ...
        //netty server start 同 name server 启动流程
        if (this.remotingServer != null) {
        ...
        //重要 核心组件让broker通过netty client 去发送请求消息给别人
        //比如broker发送请求到name server 去注册和发送心跳信息
        if (this.brokerOuterAPI != null) {
        ...
        //后台定时任务
        //!!!broker发送请求到name server 去注册  定时任务  第一次注册后  30s/每次  发送注册心跳到name server
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            ...
            //private int registerNameServerPeriod = 1000 * 30;  每隔30s发送一次心跳注册信息
    ...
    //注册场景  broker注册到name server的过程
    /**
     * 只有完成了注册，NameServer才能知道集群里有哪些Broker，
     * 然后Producer和Consumer才能找NameServer去拉取路由数据，
     * 他们才知道集群里有哪些Broker，才能去跟Broker进行通信！
     * */
